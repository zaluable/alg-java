...
/**
 * Reads in a text file and checks to see if the parentheses are balanced.No need to care the order
 * [()]{}{[()()]()} return true
 * [(]) return false
 * Character
 * 大括号-brace
...
public class ParenthesesCheck_byBook_1_3_4 {
	...
	public boolean matchParen(String target){
		...
		for (int i = 0 ; i < target.length() ; i++){
			...
			if (targetChar == LEFT_BRACE) stack.push(targetChar);
			else if (targetChar == LEFT_BRACKET) stack.push(targetChar);
			else if (targetChar == LEFT_PAREN) stack.push(targetChar);
			
			else if (targetChar == RIGHT_BRACE){
				if (stack.isEmpty()) return false;
				else if (stack.pop() != LEFT_BRACE) return false;
			}
			else if (targetChar == RIGHT_BRACKET){
				if (stack.isEmpty()) return false;
				else if (stack.pop() != LEFT_BRACKET) return false;
			}
			else if (targetChar == RIGHT_PAREN){
				if (stack.isEmpty()) return false;
				else if (stack.pop() != LEFT_PAREN) return false;
			}
		}
		return stack.empty();
	}
